...
rt_err_t rt_mutex_release(rt_mutex_t mutex)
{
	...
	if (rt_object_put_hook != RT_NULL)
		rt_object_put_hook(&(mutex->parent.parent));
#endif
...
#ifdef RT_USING_HOOK
	if (rt_object_put_hook != RT_NULL)
		rt_object_put_hook(&(event->parent.parent));
#endif
...
rt_err_t rt_event_recv(rt_event_t event, rt_uint32_t set, rt_uint8_t option, rt_int32_t timeout, rt_uint32_t *recved)
{
	...
	if (rt_object_trytake_hook != RT_NULL)
		rt_object_trytake_hook(&(event->parent.parent));
#endif
	...
	if (rt_object_take_hook != RT_NULL)
		rt_object_take_hook(&(event->parent.parent));
#endif
...
#ifdef RT_USING_HOOK
		if (rt_object_take_hook != RT_NULL)
			rt_object_take_hook(&(mb->parent.parent));
#endif
...
#ifdef RT_USING_HOOK
	if (rt_object_put_hook != RT_NULL)
		rt_object_put_hook(&(mq->parent.parent));
#endif
...
#ifdef RT_USING_HOOK
	if (rt_object_take_hook != RT_NULL)
		rt_object_take_hook(&(mq->parent.parent));
#endif
