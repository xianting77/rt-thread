...
{
	rtgui_thread_t* thread = (rtgui_thread_t*)rt_malloc(sizeof(rtgui_thread_t));
...
extern struct rt_thread* rt_thread_find(char* name);
struct rt_thread* rtgui_thread_get_server()
{
...
rt_err_t rtgui_thread_send_sync(struct rt_thread* tid, rtgui_event_t* event, rt_size_t event_size)
{
	...
	if(ack_status != RT_EOK) r = -RT_ERROR;
	else r = RT_EOK;
...
rt_err_t rtgui_thread_recv_nosuspend(rtgui_event_t* event, rt_size_t event_size)
{
	...
	r = rt_mq_recv(thread->mq, event, event_size, 1);
...
rt_err_t rtgui_thread_recv_filter(rt_uint32_t type, rtgui_event_t* event, rt_size_t event_size)
{
	...
	while(rt_mq_recv(thread->mq, event, event_size, RT_WAITING_FOREVER) == RT_EOK)
	{
		...
		else
		{
			...
			if(thread->widget != RT_NULL && RTGUI_WIDGET_EVENT_HANDLE(thread->widget) != RT_NULL)
			{
				RTGUI_WIDGET_EVENT_CALL(thread->widget, event);
			}
		}
	}
