...
void rtgui_dc_draw_border(struct rtgui_dc* dc, rtgui_rect_t* rect, int flag)
{
	...
	case RTGUI_BORDER_RAISE:
		...
		rtgui_dc_draw_shaded_rect(dc, &r, default_background, dark_grey);
		break;

	case RTGUI_BORDER_SUNKEN:
		rtgui_dc_draw_shaded_rect(dc, &r, dark_grey, white);
		rtgui_rect_inflate(&r, -1);
		rtgui_dc_draw_shaded_rect(dc, &r, dark, default_background);
		break;
	...
	case RTGUI_BORDER_DOWN:
		...
		rtgui_dc_draw_shaded_rect(dc, &r, dark, white);
